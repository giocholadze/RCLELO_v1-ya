          ...
          <div className="flex justify-between items-center h-full">
            <Link href="/" className="flex items-center hover:scale-105 transition-transform">
              <img
                ...
                className="w-9 h-9 rounded-full mr-3 shadow-md object-cover"
              />
            ...
            <nav className="hidden md:flex space-x-6">
              <Link href="/" className="font-medium hover:text-red-500 transition-colors text-sm">
                მთავარი გვერდი
              </Link>
              <div className="relative group">
                <Link href="/mens-league" className="font-medium hover:text-red-500 transition-colors text-sm">
                  კაცთა ლიგა
                </Link>
                <div className="absolute top-full left-0 mt-1 bg-background border rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 min-w-48">
                  <Link href="/mens-league" className="block px-4 py-2 text-sm hover:bg-muted">
                    ლიგის ინფო
                  </Link>
                  <Link href="/mens-players" className="block px-4 py-2 text-sm hover:bg-muted">
                    მოთამაშეები
                  </Link>
                </div>
              </div>
              <div className="relative group">
                <Link href="/youth-league" className="font-medium hover:text-red-500 transition-colors text-sm">
                  ახალგაზრდული ლიგა
                </Link>
                <div className="absolute top-full left-0 mt-1 bg-background border rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 min-w-48">
                  <Link href="/youth-league" className="block px-4 py-2 text-sm hover:bg-muted">
                    ლიგის ინფო
                  </Link>
                  <Link href="/youth-players" className="block px-4 py-2 text-sm hover:bg-muted">
                    მოთამაშეები
                  </Link>
                </div>
              </div>
              <Link href="/news" className="font-medium hover:text-red-500 transition-colors text-sm">
                სიახლეები
              </Link>
              <Link href="/gallery" className="font-medium hover:text-red-500 transition-colors text-sm">
                გალერეა
              </Link>
              <Link href="/staff" className="font-medium hover:text-red-500 transition-colors text-sm">
                პერსონალი
              </Link>
            </nav>
          ...
          <>
            ...
            <div className="fixed top-0 left-0 h-full w-80 max-w-[85vw] bg-background border-r shadow-2xl z-50 md:hidden overflow-y-auto animate-in slide-in-from-left duration-300">
              ...
              <div className="flex items-center justify-between p-4 border-b">
                ...
                <Button variant="ghost" size="sm" onClick={closeMobileMenu} className="h-8 w-8 p-0">
                  <X className="h-5 w-5" />
                </Button>
              </div>
              ...
              <div className="p-4">
                      ...
                      {item.subItems.length > 0 ? (
                        <>
                          ...
                          >
                            ...
                            <div
                              className={`transition-transform duration-200 ${expandedItems[item.title] ? "rotate-90" : ""}`}
                            >
                            ...
                            <div className="ml-4 space-y-1 pt-1">
                              {item.subItems.map((subItem, subIndex) => (
                                <Link
                                  key={subIndex}
                                  href={subItem.href}
                                  className="block p-2 text-sm text-muted-foreground hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 rounded transition-all duration-200 transform hover:translate-x-1"
                                  onClick={closeMobileMenu}
                                >
                                  {subItem.title}
                                </Link>
                ...
                <div className="mt-8 pt-6 border-t space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium">თემა</span>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={toggleTheme}
                      className="h-8 transition-all duration-200 bg-transparent"
                    >
                      {isDark ? (
                        <>
